...
public class Carro {
    ...
    private Carro(Motorista motorista, String placa, Cor cor) {
        if(!validar(motorista,placa,cor)){ throw new NullPointerException();}
        this.motorista = motorista;
        this.placa = placa;
        this.cor = cor;
    }

    private boolean validar(Motorista motorista, String placa, Cor cor) {
        if(Objects.isNull(cor)){throw new NullPointerException();}
        if(Objects.isNull(placa) || placa.isEmpty()){throw new NullPointerException();}
        if(Objects.isNull(motorista)){throw new EstacionamentoException("Não pode carro autônomo!");}
        return true;
    }
