...
public class Motorista {
    ...
    private Motorista(String nome, int idade, int pontos, String habilitacao) {
        if(!validar(nome,idade,pontos,habilitacao)){throw new NullPointerException();}
        this.nome = nome;
        this.idade = idade;
        this.pontos = pontos;
    ...
    private boolean validar(String nome, int idade, int pontos, String habilitacao){
        if(Objects.isNull(nome) || nome.isEmpty()){throw new NullPointerException();}
        if(Objects.isNull(habilitacao) || habilitacao.isEmpty()){throw new NullPointerException();}
        if(idade < 0){throw new IllegalArgumentException();}
        if(pontos < 0){throw new IllegalArgumentException();}
        return true;
    }
